...
信息流检测得到至少两个可靠拍间隔后，最新活动行左缘会成为判定线。下一拍按 `lastHit + period` 预测，音符提前 `clamp(period × 0.75, 350ms, 900ms)` 从行尾飞向左侧，并在预测拍到达判定线。判定音符与活动文字统一使用中高频信息流时钟，Deep Diving 则继续独立使用低频鼓点时钟，两套节奏不在同一行叠加。真实脉冲落入动态判定窗时按检测置信度显示 GOOD、GREAT 或 PERFECT，连续命中累积 Combo、七位分数和局部加分弹字；预测拍超时未检测到脉冲则显示一次 MISS 并重置 Combo。
